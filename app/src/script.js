...
function keyPack(keypairs) {
  let ret = []
  for (let i = 0; i < keypairs.length; i = i + 2) {
    ret.push([keypairs[i], keypairs[i + 1]])
  }
  return ret
}
...
function getKeypairs() {
  return new Promise(resolve => {
    app
      .call('getKeys')
      .first()
      .subscribe(res => resolve(keyPack(res)))
  })
}
